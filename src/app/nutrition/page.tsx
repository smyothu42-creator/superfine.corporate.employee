...
import { Apple, LoaderCircle, ArrowLeft, ChevronDown, Search, Check, SlidersHorizontal, X } from "lucide-react";
...
/**
 * In-app nutrition lookup — a standalone screen (no account or order needed)
 * styled to match the rest of the app rather than the sign-in shell. Everything
 * stays inside the app: pick an item, resolve its options (a combo's label
 ...
 * order, and there are no links out of the app.
 */
  ...
  return (
    <div className="relative isolate flex min-h-dvh flex-col bg-background">
      {/* A hand-drawn food-doodle wash so the page reads as designed space
          ...
          the page background) so `main` needn't create a stacking context that
          would trap modals below the sticky header. Shared with the sign-in screen. */}
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        <FoodDoodles />
      </div>

      {/* App-consistent top bar: back to the menu + screen title, mirroring the
          in-app topbar rather than the login hero. */}
...
function NutritionLookup() {
  ...
  const [selectedId, setSelectedId] = React.useState(
    initial && getItem(initial)?.type === "individual" ? initial : "",
  );
